...
from big_vision.models import modula_attn
import flax
...
class MlpBlock(nn.Module):
  ...
  def __call__(self, x, deterministic=True):
    ...
    x = nn.gelu(x) / 1.1289  # 1.1289 is the max derivative of gelu(x)
...
class Encoder1DBlock(nn.Module):
  ...
  def __call__(self, x, deterministic=True):
    ...
    x = out["+sa"] = (1.0 - self.residual_scale) * x + self.residual_scale * y / 3.0
    ...
    y = nn.with_logical_constraint(y, ("act_batch", "act_len", "act_emb"))
    y = nn.Dropout(rate=self.dropout)(y, deterministic)
    x = out["+mlp"] = (1.0 - self.residual_scale) * x + self.residual_scale * y
    x = nn.with_logical_constraint(x, ("act_batch", "act_len", "act_emb"))
    return x, out
    ...
    if self.scan:
      ...
      x, scan_out = nn.scan(
          ...
          length=self.depth)(
              ...
              residual_scale=1/(2 * self.depth))(x, deterministic)
      ...
      for lyr in range(self.depth):
        block_cur = Encoder1DBlock(
            ...
            dropout=self.dropout, residual_scale=1/(2 * self.depth))
